"""Client de test pour l'API FastAPI du modele de detection de fraude.

Seance 15 - TP Tests de l'API
    Envoie quelques payloads de test a une instance locale de l'API
    et affiche les reponses de /health, /predict et /model-info.

Lancement :
    uv run python -m src.scripts.predict_fraud
    uv run python -m src.scripts.predict_fraud --url http://127.0.0.1:8000
"""
...
def main() -> None:
    ...
    with httpx.Client(base_url=args.url, timeout=60.0) as client:
        ...
        # Test du endpoint /predict pour chaque payload
