...
def build_model_specs() -> list[ModelSpec]:
    ...
    return [
        ModelSpec(
            ...
            build_estimator=lambda params: cast(ClassifierMixin, RandomForestClassifier(random_state=RANDOM_STATE, **params)),
        ),
        ModelSpec(
            name="xgboost",
            suggest_params=lambda trial: {
            ...
            build_estimator=lambda params: cast(ClassifierMixin, XGBClassifier(random_state=RANDOM_STATE, eval_metric="logloss", n_jobs=-1, **params)),
        ),
        ModelSpec(
            name="lightgbm",
            suggest_params=lambda trial: {
...
def describe_registered_version(
    ...
    description = (
        f"Famille d'algorithme : {result.spec.name}\n\n"
        f"**Metriques de Test :**\n"
        f"- CV ROC AUC : {result.study.best_value:.3f}\n"
        f"- Test ROC AUC : {result.test_roc_auc:.3f}\n\n"
        f"**Optuna :**\n"
        f"- Sampler : TPE\n"
        f"- Nombre d'essais : {n_trials}\n\n"
        f"**Hyperparametres optimaux :**\n"
        f"{result.study.best_params}"
    )
    ...
    tags = {
        ...
        "cv": str(cv),
        "cv_roc_auc": str(result.study.best_value),
        "test_roc_auc": str(result.test_roc_auc),
    }
